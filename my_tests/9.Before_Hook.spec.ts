// THIS PART IS DEDICATED FOR USING HOOKS IN TESTS CREATION
...
test.describe('Home Page Tests', () => {
    // !!! Before we start any test we can use beforeEach Hook to open the proper page for exmaple
    test.beforeEach(async ({ page }) => {
        // Firstly we need to declare object instance in here which is used before Each test
        homePage = new HomePage(page);
        // Here we can implement code which will be executed before each of the implemented tests
        await homePage.navigate(); //--> now the code can be removed from the specific tests
    })
    ...
    test('Open HomePage and verify the title', async ({ page }) => {
        ...
        // homePage = new HomePage(page); // --> it can be removed from here when passed to teh beforeEach Hook // Not necessarily in here it can be removed from here when passed to teh beforeEach Hook
        // Now we can start to add content of our test
        // 1. Open the page we want to test
        //   await page.goto('https://practice.sdetunicorns.com/');
        // await homePage.navigate(); //--> this part of code can be easily removed when we are using beforeEach Hooks.
    ...
    test('Clicking get Started Button', async ({ page }) => {
        // !! The first thing in hetre we need to create new page constructor based on our class befor start the test creation
        // homePage = new HomePage(page); // --> it can be removed from here when passed to teh beforeEach Hook
        // 1. Open the page
        //   await page.goto('https://practice.sdetunicorns.com/');
        // await homePage.navigate(); //--> this part of code can be easily removed when we are using beforeEach Hooks.
    ...
    test('Verify Search icon is visible using xpath selector and css selector', async () => {
        // homePage = new HomePage(page); // --> it can be removed from here when passed to teh beforeEach Hook
        // 1. Go to proper page
        // await page.goto('https://practice.sdetunicorns.com/');
        // await homePage.navigate(); //--> this part of code can be easily removed when we are using beforeEach Hooks.
